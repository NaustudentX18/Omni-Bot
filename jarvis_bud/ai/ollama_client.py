...
class OllamaClient:
    ...
    def __init__(
        self, base_url: str = "http://localhost:11434", model: str = "gemma2:2b-instruct-q4_K_M"
    ):
    ...
    async def generate_async(
        ...
        try:
            payload = {"model": self.model, "prompt": prompt, "stream": True, "temperature": 0.7}
            ...
            response = await loop.run_in_executor(
                ...
                lambda: requests.post(
                    f"{self.base_url}/api/generate", json=payload, stream=True, timeout=30
                ),
            )
