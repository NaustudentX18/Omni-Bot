...
class ButtonHandler:
    ...
    def __init__(self, button_a_gpio=17, button_b_gpio=27, long_press_seconds: float = 2.0):
        ...
        self._button_a_callback: Callable | None = None
        self._button_b_callback: Callable | None = None
        self._button_b_long_callback: Callable | None = None
    ...
    def _try_initialize(self):
        ...
        try:
            ...
            GPIO.add_event_detect(
                self.button_a_gpio, GPIO.FALLING, callback=self._handle_button_a, bouncetime=300
            )
            GPIO.add_event_detect(
                self.button_b_gpio, GPIO.FALLING, callback=self._handle_button_b, bouncetime=80
            )
        ...
        with self._callback_lock:
            ...
            if not self._use_mock:
                try:
                    ...
                    while GPIO.input(self.button_b_gpio) == 0 and press_duration < (
                        self._long_press_seconds + 1.0
                    ):
                        time.sleep(0.02)
